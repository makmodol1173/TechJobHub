...
def dashboard(request):
    ...
    if recruiter_id:
        with connection.cursor() as cursor:
            query_jobs = "SELECT title, description, location, key_responsibilities, educational_requirement, year_of_experience, deadline, type FROM job_post WHERE recruiter_id = %s"
            cursor.execute(query_jobs, (recruiter_id,))
            job_posts = cursor.fetchall()
            has_posts = bool(job_posts)

    context = {'job_posts': job_posts, 'has_posts': has_posts}
